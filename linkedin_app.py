...
ss = pd.DataFrame({
    "sm_li": clean_sm(s["web1h"]),                         # LinkedIn use (target)
    "income": s["income"].where(s["income"] <= 9),         # cap at 9
    "education": s["educ2"].where(s["educ2"] <= 8),        # cap at 8
    "parent": clean_sm(s["par"]),                          # parent or not
    "married": np.where(s["marital"] == 1, 1, 0),          # married = 1
    "female": np.where(s["gender"] == 2, 1, 0),            # female = 1
    "age": s["age"].where(s["age"] <= 98)                  # cap age at 98
})
...
st.sidebar.header("Enter Person Characteristics")
...
income = income_options.index(income_label) + 1   # returns numerical value 1–9

education_label = st.sidebar.selectbox("Education level", education_options)
education = education_options.index(education_label) + 1   # returns numerical value 1–8
...
# Convert marital choice to binary:
